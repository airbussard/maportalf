  ...
  try {
    ...
    if (tokenError || !tokenData) {
      ...
      return NextResponse.redirect(
        new URL('/mayday/confirmed?status=invalid', request.url)
      )
    }
    ...
    if (tokenData.confirmed) {
      ...
      return NextResponse.redirect(
        new URL('/mayday/confirmed?status=already', request.url)
      )
    }
    ...
    if (new Date(tokenData.expires_at) < new Date()) {
      ...
      return NextResponse.redirect(
        new URL('/mayday/confirmed?status=expired', request.url)
      )
    }
    ...
    if (updateError) {
      ...
      return NextResponse.redirect(
        new URL('/mayday/confirmed?status=error', request.url)
      )
    }
    ...
    return NextResponse.redirect(
      new URL(`/mayday/confirmed?status=success&type=${tokenData.action_type}`, request.url)
    )
